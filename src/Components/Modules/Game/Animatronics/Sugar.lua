    ...
    if SugarAI.active then
        if SugarAI.currentState <= 3 then
            ...
            if SugarAI.timer >= 9.7 then
                ...
                if SugarAI.move <= NightState.animatronicsAI.sugar and NightState.animatronicsAI.sugar > 0 and not officeState.hasAnimatronicInOffice then
        ...
        elseif SugarAI.currentState == 4 then
            ...
            if not officeState.hasAnimatronicInOffice then
                if SugarAI.patience >= 350 and not officeState.vent.left then
                    if not NightState.killed then
                        NightState.killed = true
                        jumpscareController:init("sugar", 35)
                        jumpscareController.onComplete = function()
                            NightState.KilledBy = "sugar"
                            gamestate.switch(DeathState)
                        end
                    end
                elseif SugarAI.patience >= 350 and officeState.vent.left then
