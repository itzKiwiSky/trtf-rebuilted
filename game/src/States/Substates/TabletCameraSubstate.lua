...
function TabletCameraSubState:update(elapsed)
    ...
    if NightState.AnimatronicControllers["kitty"] ~= nil or NightState.AnimatronicControllers["sugar"] ~= nil then
        ...
        if (self.camID == "vent_kitty" or self.camID == "vent_sugar") and NightState.officeState.nightRun then
    ...
    if love.mouse.isDown(1) then
        if NightState.officeState.tabletUp then
            if collision.pointRect({ x = mx, y = my }, self.clickArea) then
                ...
                if not AudioSources["sfx_buzzlight"]:isPlaying() then
...
function TabletCameraSubState:mousepressed(x, y, button)
    ...
    if button == 1 then
        ...
        if self.camID == "prize_corner" then
            local hb = {
                x = 700,
                y = 390,
                w = 170,
            ...
            if collision.pointRect({ x = mx, y = my }, hb) then
                if not NightState.officeState.minhaMaeToco then
                    NightState.myMom:setVolume(0.35)
                    NightState.myMom:play()
                    NightState.officeState.minhaMaeToco = true
                end
            end
        end
    end
