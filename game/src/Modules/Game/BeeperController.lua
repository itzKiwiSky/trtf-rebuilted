local BeeperController = {}

function BeeperController:init(frames, speed, k)
    self.key = k
    self.frames = frames
    self.tabUp = false
...
function BeeperController:setState(closed)
    if not self.animationRunning then
        _playAnimation(self, not closed)
    end
end

function BeeperController:draw()
...
function BeeperController:update(elapsed)
