...
local function kill()
    if not NightState.killed then
        ...
        NightState.jumpscareController.id = "foxy"
...
function FoxyAI.update(elapsed)
    if FoxyAI.currentState <= 1 then
        ...
        if FoxyAI.timer >= 7.3 then
            ...
            if FoxyAI.move <= NightState.animatronicsAI.foxy and NightState.animatronicsAI.foxy > 0 and not NightState.officeState.hasAnimatronicInOffice then
